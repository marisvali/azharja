...
public:
  MutexTryLocker(QRecursiveMutex &mutex)
      : mMutex(mutex), mLocked(mutex.tryLock()) {}
  ~MutexTryLocker() {
    if (mLocked)
      mMutex.unlock();
  }
...
Item *Item::LoadFromDisk(QString pathXml, Data &parent) {
  ...
  if (!fileMeta.open(QIODevice::ReadOnly | QIODevice::Text))
    throw new std::runtime_error(
        ("Invalid path: " + pathXml).toUtf8().constData());
  ...
  if (!fileJournal.open(QIODevice::ReadOnly | QIODevice::Text))
    throw new std::runtime_error(
        ("Invalid path: " + pathJournal).toUtf8().constData());
  ...
  if (!fileAnswer.open(QIODevice::ReadOnly | QIODevice::Text))
    throw new std::runtime_error(
        ("Invalid path: " + pathAnswer).toUtf8().constData());
...
void Item::SaveToDisk(QString path) {
  // Meta info.
  {
    ...
    if (mDirtyMeta) {
      ...
      QFile fileMeta(pathXml);
      if (!fileMeta.open(QIODevice::WriteOnly | QIODevice::Truncate |
                         QIODevice::Text))
        throw new std::runtime_error(
            ("Invalid path: " + pathXml).toUtf8().constData());
  ...
  // Journal.
  {
    ...
    if (mDirtyJournal) {
      QString pathJournal =
          QDir(path).filePath(QString::number(mID) + "-J.txt");
      QFile fileJournal(pathJournal);
      if (!fileJournal.open(QIODevice::WriteOnly | QIODevice::Truncate |
                            QIODevice::Text))
        throw new std::runtime_error(
            ("Invalid path: " + pathJournal).toUtf8().constData());
  ...
  // Answer.
  {
    ...
    if (mDirtyAnswer) {
      ...
      QFile fileAnswer(pathAnswer);
      if (!fileAnswer.open(QIODevice::WriteOnly | QIODevice::Truncate |
                           QIODevice::Text))
        throw new std::runtime_error(
            ("Invalid path: " + pathAnswer).toUtf8().constData());
